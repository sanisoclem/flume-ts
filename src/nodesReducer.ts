...
const addConnection = (nodes: any, input: any, output: any, portTypes: any) => {
  const newNodes = {
    ...nodes,
    [input.nodeId]: {
      ...nodes[input.nodeId],
...
const removeConnection = (nodes: any, input: any, output: any) => {
...
const getFilteredTransputs = (transputs: any, nodeId: any) =>
  Object.entries(transputs).reduce((obj: any, [portName, transput]: any) => {
    const newTransputs = transput.filter((t: any) => t.nodeId !== nodeId);
...
const reconcileNodes = (initialNodes: any, nodeTypes: any, portTypes: any, context: any) => {
  ...
  let nodesToDelete = Object.values(nodes)
    .map((node: any) => (!nodeTypes[node.type] ? node.id : undefined))
    .filter(x => x);

  nodesToDelete.forEach((nodeId: any) => {
    nodes = nodesReducer(
      nodes,
      {
        type: "REMOVE_NODE",
  ...
  let reconciledNodes: any = Object.values(nodes).reduce((nodesObj: any, node: any) => {
  ...
  reconciledNodes = Object.values(reconciledNodes).reduce((nodesObj: any, node: any) => {
    let newNode = { ...node };
    const nodeType = nodeTypes[node.type];
...
export const getInitialNodes = (
  ...
  return {
    ...reconciledNodes,
    ...defaultNodes.reduce((nodes: any, dNode: any, i: any) => {
      const nodeNotAdded = !Object.values(initialNodes).find(
        (n: any) => n.type === dNode.type
      );
      if (nodeNotAdded) {
        nodes = nodesReducer(
          nodes,
...
const nodesReducer = (
  nodes: any,
  action: any = {},
  { nodeTypes, portTypes, cache, circularBehavior, context }: any,
  dispatchToasts: any = undefined
) => {
  switch (action.type) {
    case "ADD_CONNECTION": {
    ...
    case "DESTROY_TRANSPUT": {
      ...
      return connections.reduce((nodes: any, cnx: any) => {
...
export const connectNodesReducer = (reducer: any, environment: any, dispatchToasts: any) => (
  state: any,
  action: any
