...
const buildControlType =
  (defaultConfig: any, validate = (e: any) => {}, setup = (e: any) => ({})) =>
  (config: any) => {
    validate(config);
    return {
...
export const getPortBuilders = (ports:any) =>
  Object.values(ports).reduce((obj:any, port:any) => {
    obj[port.type] = (config:any = {}) => {
      return {
        type: port.type,
...
export class FlumeConfig {
  ...
  removePortType(type:any, { skipDynamicNodesCheck = false } = {}) {
      ...
      const affectedNodes = Object.values(this.nodeTypes).filter(
        (node:any) =>
          (Array.isArray(node.inputs) &&
            node.inputs.find((p:any) => p.type === type)) ||
          (Array.isArray(node.outputs) &&
            node.outputs.find((p:any) => p.type === type))
      );
