...
function useTransputs (transputsFn: any, transputType: any, nodeId: any, inputData: any, connections: any) {
  ...
  React.useEffect(() => {
    ...
    for (const transput of prevTransputs) {
      const current = transputs.find(({ name }: any) => transput.name === name);
      if (!current) {
        nodesDispatch({
    ...
    <div className={styles.wrapper} data-flume-component="ports">
      ...
      {!!resolvedOutputs.length && (
        <div className={styles.outputs} data-flume-component="ports-outputs">
          {resolvedOutputs.map((output: any) => (
            <Output
              {...output}
              triggerRecalculation={triggerRecalculation}
              inputTypes={inputTypes}
...
const Port = ({
  ...
  const dragStartCoordinatesCache = React.useRef<any>(dragStartCoordinates);
  const port = React.useRef<any>();
  const line = React.useRef<any>();
  const lineInToPort = React.useRef<any>();
