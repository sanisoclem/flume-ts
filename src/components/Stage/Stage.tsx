...
const Stage = ({
  ...
  const handleWheel = React.useCallback(
    (e: any) => {
      ...
      dispatchStageState(({ scale: currentScale, translate: currentTranslate }: any) => {
  ...
  const handleDragDelayStart = (e:any) => {
  ...
  const handleMouseDrag = (coords: any, e: any) => {
  ...
  const handleDragEnd = (e: any) => {
    ...
    dispatchStageState(({ translate: tran }: any) => ({
      type: "SET_TRANSLATE",
      translate: {
        x: tran.x + xDistance,
        y: tran.y + yDistance
  ...
  const addNode = ({ node, internalType }: any) => {
    const wrapperRect = wrapper.current!.getBoundingClientRect();
    const x =
  ...
  const handleDocumentKeyUp = (e: any) => {
    if (e.which === 32) {
    ...
    () => {
      const options: any[] = orderBy(
        Object.values(nodeTypes)
          .filter((node: any) => node.addable !== false)
          .map((node: any) => ({
            value: node.type,
            label: node.label,
